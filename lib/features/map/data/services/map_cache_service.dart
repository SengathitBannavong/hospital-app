...
class MapCacheService {
  ...
  static const _edgeParseIsolateThreshold = 1000;

  Future<Box<dynamic>>? _boxFuture;
  final _edgeMemoryCache = <int, List<MapEdge>>{};
  ...
  static List<MapEdge> _parseEdgesPayload(Object? raw) {
    Object? decoded;
    if (raw is String) {
      decoded = jsonDecode(raw);
    } else {
    ...
    if (decoded is! List) return const <MapEdge>[];
    return decoded
        .whereType<Map>()
  ...
  static List<Map<String, dynamic>> _serializeEdgesPayload(
    List<MapEdge> edges,
  ) {
    return edges.map((edge) => edge.toJson()).toList(growable: false);
  }
